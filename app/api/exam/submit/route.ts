...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: updateError } = await supabase
      .from('exam_sessions')
      .update({
        final_cheat_score: finalCheatScore,
    ...
    const resultData = {
      ...
      focus_score: finalCheatScore,  // Store inverted focus score (0-100, 100=good)
    ...
    return NextResponse.json({
      success: true,
      final_cheat_score: finalCheatScore,  // This is the focus score to display
      focus_score: finalCheatScore,
      status: proctoringStatus === 'flagged' ? 'flagged' : 'submitted',
